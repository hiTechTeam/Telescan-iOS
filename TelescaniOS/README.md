...
- **Telegram Integration**: Seamless connection to existing Telegram accounts
...
- **Battery Optimized**: Efficient BLE scanning with power management
...
- **View Models**: Manages app state and business logic
...
1. Download Telescan from the App Store _(link to be added)_
2. Open Telegram and find @TelescanBot
...
```bash
git clone https://github.com/yourusername/Telescan-iOS.git
cd Telescan-iOS
```
...
- `GET /api/tunnel?hashed_code={hash}` - Get user data by hashed code
- `GET /api/getuser?tgId={id}` - Get user data by Telegram ID
- `POST /api/updatephoto` - Update profile photo

### Authentication

All API calls use SHA256 hashed authentication codes. The code is generated by the Telegram bot and must be entered manually in the app.
...
- Grant location permissions (required for BLE on iOS)
...
- Confirm @TelescanBot is accessible
...
- Inspired by privacy-focused social applications
